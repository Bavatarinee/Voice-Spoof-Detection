...
def extract_features_from_audio(y: np.ndarray, sr: int = SR, channels: int = 3) -> np.ndarray:
    """Extract 40-MFCC features + optional Delta + Delta-Delta — mirrors app.py / train_model.py."""
    if sr != SR:
    ...
    if channels == 3:
        # Delta and Delta2 captures spectral change over time (3 channels)
        delta_mfcc = librosa.feature.delta(mfcc)
        delta2_mfcc = librosa.feature.delta(mfcc, order=2)
        features = np.stack([mfcc, delta_mfcc, delta2_mfcc], axis=-1)
    else:
        # Only the base MFCC channel (1 channel)
        features = mfcc[..., np.newaxis]
        
    return features  # shape (40, 94, channels)
...
# ─────────────────────────────────────────────────────────────
def load_folder_raw(folder_name: str, label: int, channels: int = 3):
    ...
    for fname in files:
        ...
        try:
            ...
            feat = extract_features_from_audio(y_audio, sr, channels=channels)
            samples.append(feat)
            labels.append(label)
...
def build_dataset(channels: int = 3):
    ...
    for folder in spoof_folders:
        s, l = load_folder_raw(folder, SPOOF_LABEL, channels=channels)
        X.extend(s); y_labels.extend(l)

    X_arr = np.array(X)
    y_arr = np.array(y_labels)
    # X_arr is already (-1, 40, 94, channels)
    return X_arr, y_arr
...
def main():
    ...
    # Detect required channels from model input shape
    expected_channels = model.input_shape[-1]
    print(f"🔍 Model input shape: {model.input_shape}")
    print(f"🔍 Detected expected channels: {expected_channels}")

    print("\n📝 Building evaluation dataset (no augmentation)…")
    X_all, y_all = build_dataset(channels=expected_channels)
