...
def extract_features_from_audio(y: np.ndarray, sr: int = SR) -> np.ndarray:
    """
    Extract 40-MFCC features + Delta + Delta-Delta (3 channels).
    ...
    # Delta and Delta-Delta MFCCs (captures spectral changes over time)
    delta_mfcc = librosa.feature.delta(mfcc)
    delta2_mfcc = librosa.feature.delta(mfcc, order=2)

    # Stack into 3 channels (N_MFCC, N_FRAMES, 3)
    features = np.stack([mfcc, delta_mfcc, delta2_mfcc], axis=-1)
    
    return features  # (40, 94, 3)
...
#  PREPARE DATA
# ─────────────────────────────────────────────
# Reshape for CNN → (N, 40, 94, 3)
X_arr = np.array(X)
# X is already (N, 40, 94, 3) from the new extract_features_from_audio
...
model = Sequential([
    # Block 1
    Conv2D(32, (3, 3), activation='relu', padding='same', input_shape=(N_MFCC, N_FRAMES, 3)),
    BatchNormalization(),
    MaxPooling2D((2, 2)),
    Dropout(0.25),
    ...
    # Block 4 (Added for better representation)
